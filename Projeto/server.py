...
class GameServer:
    ...
    def save_latest_scores(self):
        """Update latest scores, storing the latest 10 scores to file."""

        logger.debug("Save latest scores")
        for player in self.game_player.values():
            logger.info(
                "Saving: %s <%s>",
                player,
            ...
            self._scores.append((player, self.game.snakes[player].score))
            self._scores = self._scores[-MAX_SCORES:]

        with open(SCORES_FILE, "w") as outfile:
            json.dump(self._scores, outfile)
    ...
    async def mainloop(self):
        ...
        while True:
            ...
            try:
                ...
                game_over = {"highscores": self.save_highscores(), "scores": self.save_latest_scores()}
